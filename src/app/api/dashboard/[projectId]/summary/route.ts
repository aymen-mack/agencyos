...
export async function GET(
  ...
  const isClosed = (s: string) => s === 'closed_deal' || s === 'purchased'
  const totalPurchased = lc.filter((l) => isClosed(l.status)).length
  const convRate = totalReg > 0 ? (totalPurchased / totalReg) * 100 : 0
  const convRatePrev = totalRegPrev > 0 ? (lp.filter((l: { status: string }) => isClosed(l.status)).length / totalRegPrev) * 100 : 0
  ...
  const prevPurchased = lp.filter((l: { status: string }) => isClosed(l.status)).length
